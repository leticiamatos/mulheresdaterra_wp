...
$(window).scroll(function () {
	...
	if ( $('body').hasClass('home')){
		...
		function isVisible(){
			...
			var site_height = $(document).height() - $(window).height() - 50;
			var site_height_200 = $(document).height() - $(window).height() - 200;
			...
			if( isScrolledIntoView(block_as_mulheres_01) || isScrolledIntoView(block_as_mulheres_02) || isScrolledIntoView(block_as_mulheres_03) ){
				...
				$('#bgsection').removeClass("reach_bottom");
			...
			}else if(topo > site_height_200 || topo == site_height){
				$('#bgsection').addClass("reach_bottom");
				//alert("Mr. Previatti, você foi longe demais...");
			}
